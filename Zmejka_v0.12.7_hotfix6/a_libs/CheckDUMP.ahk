CheckDUMP(filePath, DT_RESTART_Value)
{
    ...
    if InStr(fileContent, "&DUMP")
    {
        ...
        else  ;  DT_RESTART is not present, add it to the &DUMP line
        {
            ...
            ; Ensures COLUMN_DUMP_LIMIT is added if DT_RESTART was not initially present
        }

        if !InStr(fileContent, "COLUMN_DUMP_LIMIT")
        {
            fileContent := RegExReplace(fileContent, "&DUMP\s*(.+)/", "&DUMP DT_RESTART=" DT_RESTART_Value " COLUMN_DUMP_LIMIT=.FALSE./")
            ; Add COLUMN_DUMP_LIMIT if it is not already present
        }
    }
    else
    {
        fileContent := "&DUMP DT_RESTART=" DT_RESTART_Value " COLUMN_DUMP_LIMIT=.FALSE." fileContent   ;  &DUMP is not present, add the line with DT_RESTART and COLUMN_DUMP_LIMIT
    }
